...
static uint32 balloc(uint32 dev) {
    ...
    for (b = 0; b < spb.size; b+= BPB) {
        bp = bread(dev, BBLOCK(b, spb)); 
        //这里的bp实际上是位图块, 位图块中标识了所有数据块的使用情况
        for (bi = 0; bi < BPB && b + bi < spb.size; bi++) {
            m = 1 << (bi % 8);  
            if ((bp->data[bi / 8] & m) == 0) {
                // 说明当前块的位图标识位为0，即空闲的块
                bp->data[bi / 8] |= m;  // 将位图中的那一位设为已分配
                bwrite(bp);
                brelse(bp);
                bzero(dev, b + bi);
                return b + bi;
...
static void readspb(int dev, struct superblock *spb) {
    struct buf *bp;
    bp = bread(dev, 1);
    memmove(spb, bp->data, sizeof(*spb));
    brelse(bp);
}
...
struct inode *ialloc(uint dev, short type)
{
	...
	for (inum = 1; inum < spb.ninodes; inum++) {
		bp = bread(dev, IBLOCK(inum, spb));
...
void iupdate(struct inode *ip)
{
	...
	bp = bread(ip->dev, IBLOCK(ip->inum, spb));
...
// Fine the inode with number inum on device dev
// and return the in-memory copy. Does not read it from disk
...
void ivalid(struct inode *ip) {
    ...
    if (ip->valid == 0) {
        bp = bread(ip->dev, IBLOCK(ip->inum, spb)); // bp是ip这个inode所在的磁盘块
        dip = (struct dinode *)bp->data + ip->inum % IPB; 
        //dip获取到在bp中的data下标，因为该磁盘块储存的是inode信息，所以就得到了对应的dinode结构体
        ip->type = dip->type;
        ip->size = dip->size;
